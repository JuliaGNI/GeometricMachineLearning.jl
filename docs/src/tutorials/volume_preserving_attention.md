...
```@example volume_preserving_attention
function set_up_networks(intermediate_length::Int = 3)
    model_skew = Chain(Dense(1, intermediate_length, tanh), VolumePreservingAttention(intermediate_length, 5; skew_sym = true),  Dense(intermediate_length, 1, identity))
    model_arb  = Chain(Dense(1, intermediate_length, tanh), VolumePreservingAttention(intermediate_length, 5; skew_sym = false), Dense(intermediate_length, 1, identity))
    model_comp = Chain(Dense(1, intermediate_length, tanh), Dense(intermediate_length, 1, identity))
...
```@example volume_preserving_attention 
initial_condition = dl.input[:, 1:5, 1]
