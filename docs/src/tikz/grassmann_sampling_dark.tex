...
\begin{tikzpicture}[
    ...
    \node[squarednodeR]        (samples)       [below of=fig, yshift=-2cm] {$\begin{aligned} \begin{pmatrix} x^{(1)} \\ y^{(1)} \\ z^{(1)} \end{pmatrix},  \begin{pmatrix} x^{(2)} \\ y^{(2)} \\ z^{(2)} \end{pmatrix}, \cdots, \begin{pmatrix} x^{(\mathtt{nop})} \\ y^{(\mathtt{nop})} \\ z^{(\mathtt{nop})} \end{pmatrix}\end{aligned}$};
