...
- [Geometric structure](@ref "Symplectic Systems"): structure preservation takes a prominent role in this dissertation. In general *structure* refers to some property that the analytic solution of a differential equation also has and that we want to preserve when modeling the system. Here we discuss *symplecticity* and [volume preservation](@ref "Divergence-Free Vector Fields") in detail. We also introduce neural networks in this chapter and give a definition of [geometric neural networks](@ref "Structure-Preserving Neural Networks").
