...
This section discusses the symplectic neural network (SympNet) architecture and its implementation in `GeometricMachineLearning.jl`.
...
```math
  \begin{pmatrix} q^{(t)} \\ p^{(t)} \end{pmatrix} \in \mathbb{R}^{2d}
```
the SympNet aims to compute the *next position*
```math
  \mathrm{SympNet}\left( \begin{pmatrix} q^{(t)} \\ p^{(t)} \end{pmatrix} \right) = \begin{pmatrix} \tilde{q}^{(t+1)} \\ \tilde{p}^{(t+1)} \end{pmatrix} \in \mathbb{R}^{2d}
``` 
...
The keywords `init_upper_linear` and `init_upper_act` indicate whether the first linear (respectively activation) layer is of ``q`` type[^4].

[^4]: If `init_upper_linear = true` then the first layer is of ``q`` type, i.e. changes the ``q`` component and leaves the ``p`` component unchanged. 
...
The keyword `init_upper` for [`GSympNet`](@ref) is similar as in the case for [`LASympNet`](@ref).
...
We further consider a special topology on ``C^r(U, \mathbb{R}^d)`` and the associated notion of [denseness](@ref "Basic Concepts from General Topology"):

```@eval
Main.definition(raw"Let ``m,d,r\in \mathbb{N}`` with ``m,d>0`` be given, ``U`` an open set of ``\mathbb{R}^m``, and ``I,J\subset C^r(U,\mathbb{R}^d)``. We say ``J`` is **``r``-uniformly dense on compacta in ``I``** if ``J \subset I`` and for any ``f\in I``, ``\epsilon>0``, and any compact ``K\subset U``, there exists ``g\in J`` such that ``||f-g||_{C^r(K,\mathbb{R}^{d})} < \epsilon``.")
```
...
To train the SympNet, one needs data along a trajectory such that the model is trained to perform an integration. The loss function is defined as[^5]:

[^5]: This loss function is implemented as [`FeedForwardLoss`](@ref) in `GeometricMachineLearning`.
