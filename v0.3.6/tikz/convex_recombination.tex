...
\begin{tikzpicture}[module/.style={draw, very thick, rounded corners, minimum width=15ex},
    ...
    \node[output1] (o1) {$\sum_{i=1}^Ty^{(1)}_iz^{(i)}$};
    \node[output2, left of=o1, xshift=5cm] (o2) {$\sum_{i=1}^Ty^{(2)}_iz^{(i)}$};
    \node[output3, left of=o2, xshift=5cm] (output_dots) {$\cdots$};
    \node[output4, left of=output_dots, xshift=5cm] (oT) {$\sum_{i=1}^Ty^{(T)}_1z^{(i)}$};

    \node[input, above of=o1, yshift = 4cm] (z1) {$z^{(1)}$};
    \node[input, above of=o2, yshift = 4cm] (z2) {$z^{(2)}$};
    \node[input, above of=output_dots, yshift = 4cm] (input_dots) {$\cdots$};
    \node[input, above of=oT, yshift = 4cm] (zT) {$z^{(T)}$};
