...
function H(z, c, l, Δx, link_matrix)
    N = length(z)÷2
    #momentum terms
    H_out = 0.5*Δx*sum(z[(N + 1):(2 * N)] .^ 2)
    #potential
    q = z[1:N]
    Q = repeat(q', outer = (N, 1))
    #quadratic distances
    CQ = toda_pot.(Q - Q')
    H_out = H_out + 0.5*c^2/Δx*tr(link_matrix*CQ)
    return H_out
end
...
z_init = zeros(2*N)
for i in 1:N
    z_init[i] = h(s(x_vec[i]))[1]
end
z_vec = zeros(2*N, n_steps+1)
z_vec[:, 1] = z_init

for it in 1:n_steps
    f(z) = z - z_vec[:, it] - step_size*Xₕ(0.5*(z+z_vec[:, it]), c, l, Δx, link_matrix, J_N)
...
for it in 1:n_steps
    function f₁(ξ)
        ξ - ξ_vec₁[:, it] -
        step_size*X_red(0.5*(ξ+ξ_vec₁[:, it]), c, l, Δx, link_matrix, U₁, J_N)
    end
    ξ_vec₁[:, it + 1] = nlsolve(f₁, ξ_vec₁[:, it], autodiff = :forward).zero
    function f₂(ξ)
        ξ - ξ_vec₂[:, it] -
        step_size*X_red(0.5*(ξ+ξ_vec₂[:, it]), c, l, Δx, link_matrix, Φ, J_N)
    end
    ξ_vec₂[:, it + 1] = nlsolve(f₂, ξ_vec₂[:, it], autodiff = :forward).zero
end

err₁ = 0.5*(sum((z_vec - U₁*ξ_vec₁) .^ 2, dims = 1) .^ 0.5)
err₂ = 0.5*(sum((z_vec - Φ*ξ_vec₂) .^ 2, dims = 1) .^ 0.5)
abs_val = 0.5*(sum(z_vec .^ 2, dims = 1) .^ 0.5)

p₁ = lines(vec(err₁ ./ abs_val))
p₂ = lines(vec(err₂ ./ abs_val))

#neural network part
nn_in = Chain(Gradient(2*N, 4*N), Gradient(2*N, 4*N; change_q = false),
    SymplecticStiefelLayer(2*n_m, 2*N; inverse = true), Gradient(2*n_m, 4*n_m))
nn_out = Chain(Gradient(2*n_m, 4*n_m), SymplecticStiefelLayer(2*n_m, 2*N),
    Gradient(2*N, 4*N; change_q = false), Gradient(2*N, 4*N))
nn_total = Chain(nn_in, nn_out)

ps, st = Lux.setup(Random.default_rng(), nn_total)

o = StandardOptimizer(1.0f-5)
...
function X_nn(ξ, ps_out, st_out, c, l, Δx, link_matrix, J_n)
    return J_n*gradient(
        ξ₁ -> H(
            Lux.apply(nn_out, ξ₁, ps_out, st_out)[1], c, l, Δx, link_matrix), ξ)[1]
end
...
for it in 1:n_steps
    function f(ξ)
        ξ - ξ_vec_nn[:, it] -
        step_size*X_nn(0.5*(ξ+ξ_vec_nn[:, it]), ps_out, st_out, c, l, Δx, link_matrix, J_n)
    end
