...
function get_domain(T = Float32, spacing = T(0.01), time_step = T(0.01))
    Ω = T(-0.5):spacing:T(0.5)
    I = T(0):time_step:T(1)
    Ω, I
end

function plot_time_evolution(T = Float32; spacing = T(0.01), time_step = T(0.25), μ = T(0.3))
...
function generate_data(T = Float32; spacing = T(0.01), time_step = T(0.01), μ_collection = μ_collection)
    Ω, I = get_domain(T, spacing, time_step)
...
function analytic_solution(T = Float64; N::Int = 2048, n_time_steps = 4000,
        μ_left = μ_left, μ_right = μ_right, n_μ::Int = 8)
