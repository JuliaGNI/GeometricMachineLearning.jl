...
const ics₁ = [[sin(val), 0.0, cos(val)] for val in 0.1:0.01:(2 * π)]
const ics₂ = [[0.0, sin(val), cos(val)] for val in 0.1:0.01:(2 * π)]
const ics = [ics₁..., ics₂...]
...
model₃ = VolumePreservingTransformer(
    sys_dim, seq_length; n_blocks = n_blocks, n_linear = n_linear,
    L = L, activation = resnet_activation, skew_sym = skew_sym)

model₄ = StandardTransformerIntegrator(
    sys_dim; n_heads = n_heads, transformer_dim = sys_dim, n_blocks = n_blocks,
    L = L, resnet_activation = resnet_activation, add_connection = false)
...
JLD2.save("transformer_rigid_body.jld2",
...
function numerical_solution(sys_dim::Int, t_integration::Int, timestep::Real, ics_val::Vector)
    validation_problem = odeproblem(ics_val; timespan = (0.0, t_integration),
...
function plot_validation(t_validation; nn₂ = nn₂, nn₃ = nn₃, nn₄ = nn₄,
        plot_regular_transformer = false, plot_vp_transformer = false)
    ...
    nn₂_solution = iterate(nn₂, numerical[:, 1]; n_points = Int(floor(t_validation /
                                                                      timestep)) + 1)
    nn₃_solution = iterate(nn₃, numerical[:, 1:seq_length];
        n_points = Int(floor(t_validation / timestep)) + 1, prediction_window = seq_length)
    nn₄_solution = iterate(nn₄, numerical[:, 1:seq_length];
        n_points = Int(floor(t_validation / timestep)) + 1, prediction_window = seq_length)
    ...
    lines!(ax, t_array, numerical[1, :]; label = "implicit midpoint",
    ...
    lines!(ax, t_array, nn₂_solution[1, :]; label = "feedforward",
        color = Makie.wong_colors()[3], linewidth = 2)

    if plot_vp_transformer
        lines!(ax, t_array, nn₃_solution[1, :]; label = "transformer",
            color = Makie.wong_colors()[4], linewidth = 2)
    end

    if plot_regular_transformer
        lines!(ax, t_array, nn₄_solution[1, :]; label = "standard transformer",
            color = Makie.wong_colors()[5], linewidth = 2)
    end
...
p_training_loss = let fig = Figure()
    ...
    lines!(ax, loss_array₃; label = "transformer",
        color = Makie.wong_colors()[4], linewidth = 2)

    # lines!(ax, loss_array₁; label = "attention only", color = Makie.wong_colors()[2], linewidth = 2)

    lines!(ax, loss_array₂; label = "feedforward",
        color = Makie.wong_colors()[3], linewidth = 2)

    lines!(ax, loss_array₄; label = "standard transformer",
        color = Makie.wong_colors()[5], linewidth = 2)
...
function make_validation_plot3d(t_validation::Int, nn::NeuralNetwork)
    ...
    prediction_window = typeof(nn) <:
                        NeuralNetwork{<:GeometricMachineLearning.TransformerIntegrator} ?
                        seq_length : 1

    nn₁_solution = iterate(
        nn, numerical[:, 1:seq_length]; n_points = Int(floor(t_validation / timestep)) + 1,
    ...
    fig = Figure()
    ax = Axis3(fig[1, 1]; aspect = (1.0, 1.0, 1.0),
        xlabel = L"z_1", ylabel = L"z_2", zlabel = L"z_3",
        limits = ([-1, 1], [-1, 1], [-1, 1]))
    surface!(ax, sphere(1.0, [0.0, 0.0, 0.0])...; alpha = 0.2, transparency = true)

    lines!(ax, numerical[1, :], numerical[2, :], numerical[3, :];
        label = "implicit midpoint", color = Makie.wong_colors()[1], linewidth = 2)
    lines!(ax, numerical₂[1, :], numerical₂[2, :], numerical₂[3, :];
        color = Makie.wong_colors()[1], linewidth = 2)

    lines!(ax, nn₁_solution[1, :], nn₁_solution[2, :],
        nn₁_solution[3, :]; label = "volume-preserving transformer",
        color = Makie.wong_colors()[4], linewidth = 2)
    lines!(ax, nn₁_solution₂[1, :], nn₁_solution₂[2, :], nn₁_solution₂[3, :];
        color = Makie.wong_colors()[4], linewidth = 2)
...
mkpath(joinpath(@__DIR__, "rigid_body"))
