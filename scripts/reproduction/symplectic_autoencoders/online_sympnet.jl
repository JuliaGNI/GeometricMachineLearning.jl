...
pr = hodeensemble(; timespan = (0.0, smoke_size(800.0, 5.0)), parameters = params)
...
Random.seed!(123)
psd_nn = NeuralNetwork(psd_arch, backend, Float32)
sae_nn = NeuralNetwork(sae_arch, backend, Float32)

const n_epochs = smoke_size(262144, 2)
...
axislegend(; position = (0.82, 0.75), backgroundcolor = :transparent, color = text_color)
mkpath("symplectic_autoencoder_validation")
CairoMakie.save("symplectic_autoencoder_validation/compare_errors.pdf", fig)
...
integrator_train_epochs = smoke_size(262144, 2)
integrator_batch_size = 4096

seq_length = 4
integrator_architecture = StandardTransformerIntegrator(
    reduced_dim; transformer_dim = 10, n_blocks = 3,
    n_heads = 5, L = 2, upscaling_activation = tanh)
...
integrator_nn = NeuralNetwork(integrator_architecture, backend, Float32)
...
JLD2.save("integrator_parameters.jld2", "integrator_parameters",
    integrator_nn.params |> mtc, "training loss", train_integrator_loss)

const ics_nt = (q = mtc(dl_reduced.input.q[:, 1:seq_length, 1]),
    p = mtc(dl_reduced.input.p[:, 1:seq_length, 1]))
...
function plot_validation(t_steps::Integer = 100)
    ...
    lines!(ax_val,
        psd_rs.decoder((
            q = sol_psd_reduced.s.q[t_steps], p = sol_psd_reduced.s.p[t_steps])).q,
        label = "PSD + Implicit Midpoint", color = morange)
    lines!(ax_val,
        sae_rs.decoder((
            q = sol_sae_reduced.s.q[t_steps], p = sol_sae_reduced.s.p[t_steps])).q,
        label = "SAE + Implicit Midpoint", color = mgreen)

    name = "symplectic_autoencoder_validation/symplectic_autoencoder_validation_" *
           string(t_steps)
    # axislegend(; position = (.82, .75), backgroundcolor = :transparent, color = text_color)
    CairoMakie.save(name * ".pdf", fig_val)

    @time "time stepping with transformer" time_series = iterate(
        mtc(integrator_nn), ics; n_points = t_steps, prediction_window = seq_length)
    ...
    axislegend(; position = (0.82, 0.75), backgroundcolor = :transparent, color = text_color)
    CairoMakie.save(name * "_with_nn_integrator.pdf", fig_val)
end
...
lines!(ax_reduced, reduced_data_matrix[1, :, 1] |> mtc,
    reduced_data_matrix[2, :, 1] |> mtc; color = mgreen, label = "Reduced Data")
axislegend(; position = (0.82, 0.75), backgroundcolor = :transparent, color = text_color)
mkpath("symplectic_autoencoder_validation")
CairoMakie.save("symplectic_autoencoder_validation/reduced_data.pdf", fig_reduced)
