...
using GeometricProblems.HarmonicOscillator: hodeproblem, default_parameters, tspan, Δt, hamiltonian
...
ensemble_problem = EnsembleProblem(hodeproblem().equation, tspan, Δt, [(q = q, p = p) for (q, p) in zip(q₀, p₀)], default_parameters)
...
function dummy_setup()
    ...
    gradient(ps -> norm(dummy_model₁(rand(2, 4, 1), ps)), ps₁)
    gradient(ps -> norm(dummy_model₂(rand(2, 1, 1), ps)), ps₂)
