...
# ensemble problem
initial_conditions = [
    (q = [π / i, π / j], p = [0.0, π / k]) for i=1:5, j=1:5, k=1:5
]
initial_conditions = reshape(initial_conditions, length(initial_conditions))

sympnet_paper_parameters = (l₁ = 1., l₂ = 1., m₁ = 1., m₂ = 1., g = 1.)
ensemble_problem = EnsembleProblem(hodeproblem().equation, tspan, tstep, initial_conditions, sympnet_paper_parameters)

ensemble_solution = integrate(ensemble_problem, ImplicitMidpoint())

dl_cpu = DataLoader(ensemble_solution)
const backend = ROCBackend()
const T = Float64
const dl = DataLoader(dl_cpu, backend, T)

const seq_length = 3
const transformer_dim = 40
...
nn1 = NeuralNetwork(arch1, backend, T)
# nn2 = NeuralNetwork(arch2, backend, T)
# nn3 = NeuralNetwork(arch3, backend, T)
nn4 = NeuralNetwork(arch4, backend, T)

const batch_size = 2024
const n_epochs = 1000
const η = T(1e-5)

o_method = AdamOptimizer(η)
...
nn1 = GeometricMachineLearning.map_to_cpu(nn1)
nn4 = GeometricMachineLearning.map_to_cpu(nn4)
...
function make_training_error_plot(; theme = :dark, symplectic = true)
    ...
    symplectic ? lines!(ax, loss_array4; color = mgreen, label = "SympNet", linewidth = 2) : nothing
...
const index = 1000
init_con = (q = dl_cpu.input.q[:, 1:seq_length, index], p = dl_cpu.input.p[:, 1:seq_length, index])
init_con2 = (q = dl_cpu.input.q[:, 1, index], p = dl_cpu.input.p[:, 1, index])
...
function make_validation_plot(n_steps = n_steps; theme = :dark, symplectic = true)
    ...
    lines!(ax, start_counting:n_steps, dl_cpu.input.q[1, start_counting:n_steps, index]; color = mblue, label = "Implicit midpoint", linewidth = 2)
...
for n_steps ∈ (10, 20, 30, 40, 100, 200, 300, 400, 600, 800, 1000)
    ...
    save("comparison_plots_for_various_trajectories/DoublePendulum-Validation_$(n_steps).png", fig_light)
end