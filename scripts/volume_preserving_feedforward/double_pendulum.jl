...
function setup_and_train(
        model::Union{GeometricMachineLearning.Architecture, GeometricMachineLearning.Chain},
        batch::Batch; transformer::Bool = true)
    nn₀ = NeuralNetwork(model, backend, T)
    o₀ = Optimizer(opt_method, nn₀)

    loss_array = transformer ? o₀(nn₀, dl, batch, n_epochs, transformer_loss) :
...
function numerical_solution(ics::NamedTuple, sys_dim::Int, t_integration::Int,
    ...
    for i in axes(sol.t, 1)
        numerical_solution[1:(sys_dim ÷ 2), i + 1] = sol.q[i]
    end
    for i in axes(sol.t, 1)
        numerical_solution[(sys_dim ÷ 2 + 1):sys_dim, i + 1] = sol.p[i]
    end
...
lines!(ax_validation, t_array, nn₂_solution[1, :]; label = "feedforward",
