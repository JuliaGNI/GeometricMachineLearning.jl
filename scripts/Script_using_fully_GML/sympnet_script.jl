...
Data = get_multiple_trajectory_structure(
    nameproblem; n_trajectory = 1, n_points = 1000, timestep = 0.1,
...
ax_qp = Axis(
    plt[1, 1]; title = "G-SympNet prediction for the simple pendulum", titlesize = 15,
    xlabel = L"q", ylabel = L"p", xlabelsize = 18, ylabelsize = 18,
    xticklabelsize = 10, yticklabelsize = 10)
lines!(ax_qp, data_q[:, 1], data_p[:, 1]; label = "Training data.", linewidth = 3)
lines!(
    ax_qp, q_learned[:, 1], p_learned[:, 1]; label = "Learned trajectory.", linewidth = 3)
