...
function loss(rnn, data, batch = ((1,1),(1,2)), params = rnn.params)
    s = 0
    for (i,j) in batch
        x = [[get_data(data, :q, i, y)...,get_data(data, :p, i, y)...] for y in j:(j+9)]
        y = [get_data(data, :q, i, j+10)...,get_data(data, :p, i, j+10)...]
...
plt(1:length(total_loss), total_loss)

jldsave("nn_model", model=rnn, params = rnn.params, seq_length = ssize[2])
