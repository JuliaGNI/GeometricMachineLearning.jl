...
dict_problem_H = Dict(
    ...
    :Hénon_Heiles =>
        (x -> x[3]^2/(2) + x[4]^2/(2) + x[1]^2/2 + x[2]^2/2 + (x[1]^2*x[2]-x[2]^3/3), 2)
    #:three_body_system =>(,)
)
...
function get_phase_space_data(nameproblem, q₀, p₀, timespan = (0.0, 100.0), timestep = 0.1)
...
function get_phase_space_multiple_trajectoy(
        nameproblem; singlematrix = true, n_trajectory = 1, n_points = 10,
        timestep = 0.1, qmin = -0.2, pmin = -0.2, qmax = 0.2, pmax = 0.2)

    # get the Hamiltonien corresponding to name_problem   
    H_problem, n_dim = dict_problem_H[nameproblem]

    #define timespan
    timespan=(0.0, n_points*timestep)
    ...
    for i in 1:n_trajectory
        ...
        trajectory_q[i], trajectory_p[i] = compute_phase_space(
            H_problem, q₀, p₀, timespan, timestep)
    end
...
function compute_phase_space(H_problem, q₀, p₀, timespan = (0.0, 100.0), timestep = 0.1)
    n_dim = length(q₀)
...
function get_multiple_trajectory_structure(
        nameproblem; n_trajectory = 1, n_points = 10, timestep = 0.1,
        qmin = -1.2, pmin = -1.2, qmax = 1.2, pmax = 1.2)
    ...
    for i in 1:n_trajectory
        ...
        nt = NamedTuple{(Symbol("Trajectory_"*string(i)),)}(((
            data = Data, len = n_points+1),))
...
function get_multiple_trajectory_structure_with_target(
        nameproblem; n_trajectory = 1, n_points = 10, timestep = 0.1,
        qmin = -0.2, pmin = -0.2, qmax = 0.2, pmax = 0.2)
    ...
    for i in 1:n_trajectory
        ...
        ntt = NamedTuple{(Symbol("Trajectory_"*string(i)),)}(((
            target = pre_target, len = n_points+1),))
