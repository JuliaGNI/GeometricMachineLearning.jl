...
function rewrite(fun, SV, SX, ti, nn)
    ...
    fun = Meta.parse(replace(string(fun),
        r"function .*" => string("function ∇loss_single(::", typeof(ti),
            ", ::", typeof(nn), ", sargs, nt)\n")))
end
...
function los(::TrainingIntegrator{SymplecticEulerA},
        nn::AbstractNeuralNetwork{<:HamiltonianArchitecture},
        qₙ, qₙ₊₁, pₙ, pₙ₊₁, Δt, params = nn.params)
