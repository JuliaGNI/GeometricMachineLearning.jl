...
include("utils/lnn_data.jl")

#this contains the functions for generating the plots
include("utils/plots.jl")

#load ModelingToolkit generated functions
est   = include("mt_lnn_fun/est.jl")
#field = include("mt_lnn_fun/field.jl")
loss  = include("mt_lnn_fun/loss.jl")
step  = include("mt_lnn_fun/step.jl")

#settings and common functionality
include("mt_lnn_fun/common.jl")
...
function train_mt_lnn(n_in, ld, η, runs, DT=Float64)
	#initialise weights
	model = (
...
#train network
model, data, target, total_loss = train_mt_lnn(n_in, ld, η, runs)
...
L_est(τ) = est(τ, expand(model)...)
# dH_est(τ) = field(τ, expand(model)...)

#plot results
plot_network(L, L_est, total_loss; filename="lnn_mt.png")
