...
function train(dev::Device, n_steps, batch_size)
    global data_input = Tuple(convert_to_dev(dev, data_input[i])
    for i in 1:number_data_points)
    global data_output = Tuple(convert_to_dev(dev, data_output[i])
    for i in 1:number_data_points)
    ...
    @showprogress for iteration in 1:n_steps
        ...
        training_error_array[iteration] = mapreduce(
            i -> loss(data_input[i], data_output[i], ps, st), +, 1:number_data_points)
    end
