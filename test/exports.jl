...
# from being added, and fails on an entry below that is no longer needed.
#
# The assertions below test what the loaded module defines. `src/` is where each reason string's
# search ran, but it is not everything the package can load: `ext/HDF5Ext.jl` loads alongside HDF5,
# and this test does not load HDF5. A name defined only in that extension would read as undefined
...
const UNDEFINED_EXPORTS = Dict(
    ...
    :ResidualLayer =>
        "exported at `GeometricMachineLearning.jl:187`; its definition is at " *
        "`legacy/layers/resnet.jl:6`, which nothing under `src/` includes. The " *
        "loaded layer of that shape is `ResNetLayer` (`src/layers/resnet.jl:17`)",
    ...
    :description =>
        "exported at `GeometricMachineLearning.jl:114`; `GeometricBase` defines " *
        "`description` but does not export it, so the `import` is missing",
    :timestep =>
        "exported at `GeometricMachineLearning.jl:374`; `GeometricBase` defines " *
        "`timestep` but does not export it, so the `import` is missing",
