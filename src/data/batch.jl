...
function complete_batch_size(::TrainingData{T, TrajectoryData} where {T},
        ti::TrainingMethod, bs::Tuple{Int64, Int64, Int64})
    bs
end
function complete_batch_size(::TrainingData{T, TrajectoryData} where {T},
        ti::TrainingMethod, bs::Tuple{Int64, Int64})
    (bs..., min_length_batch(ti))
end
function complete_batch_size(data::TrainingData{T, TrajectoryData} where {T},
        ti::TrainingMethod, bs::Tuple{Int64})
    (get_nb_trajectory(data), bs..., min_length_batch(ti))
end
function complete_batch_size(data::TrainingData{T, TrajectoryData} where {T}, ti::TrainingMethod, bs::Int64)
    (get_nb_trajectory(data), bs, min_length_batch(ti))
end
function complete_batch_size(data::TrainingData{T, TrajectoryData} where {T}, ti::TrainingMethod, ::Missing)
    (1, min(DEFAULT_BATCH_SIZE, min_length(data))-min_length_batch(ti)+1,
        min_length_batch(ti))
end
function complete_batch_size(::TrainingData{T, SampledData} where {T}, ::TrainingMethod, bs::Int64)
    bs
end
function complete_batch_size(data::TrainingData{T, SampledData} where {T}, ::TrainingMethod, ::Missing)
    min(DEFAULT_BATCH_SIZE, get_nb_point(data))
end
...
function get_batch(data::TrainingData{T, TrajectoryData} where {T},
        batch_size_t::Tuple{Int64, Int64, Int64}; check = true)
    ...
    for i in index_trajectory
        ...
        push!(index_qp, [(i, j)
                         for j in rand(1:(l_i - size_sequence + 1), min(l_i, batch_size)÷size_sequence)])
    end
...
function get_batch(
        data::TrainingData{T, TrajectoryData} where {T}, batch_size::Tuple{
            Int64, Int64}; kwargs...)
    get_batch(data, (batch_size..., 1); kwargs...)
end

function get_batch(data::TrainingData{T, TrajectoryData} where {T}, batch_size::Int64; kwargs...)
    get_batch(data, (get_nb_trajectory(data), batch_size, 1); kwargs...)
end
...
function check_batch_size(data::TrainingData{T, TrajectoryData} where {T},
        batch_size::Tuple{Int64, Int64, Int64})
    ...
    nb = length([i
                 for i in 1:nb_trajectory if get_length_trajectory(data, i) < size_sequence])
    if nb == nb_trajectory
        0
