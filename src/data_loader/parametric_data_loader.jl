# A `ParametricDataLoader` stores one `NamedTuple` of system parameters per trajectory, and they all
# have to agree with the element type of the data. `parameter_eltype` promotes over the leaves of one
# such `NamedTuple`; promoting over the vector as well would hide a mismatch instead of reporting it.
function _system_parameter_eltype(parameters::AbstractVector{<:NamedTuple})
    T = parameter_eltype(first(parameters))
    for p in parameters
        parameter_eltype(p) == T ||
            error("The parameters do not all have the same element type.")
    end
    T
end
...
struct ParametricDataLoader{T, AT <: QPTOAT2, VT <: AbstractVector}
    ...
    function ParametricDataLoader(data::QPTOAT2{T, 3}, parameters::AbstractVector) where {T}
        ...
        @assert T == _system_parameter_eltype(parameters) "Provided data and parameters must have the same eltype!"
        ...
        new{T, typeof(data), typeof(parameters)}(
            data, input_dim, input_time_steps, parameters, n_params)
    end
end
...
# The same solution shape `DataLoader(::EnsembleSolution)` takes: since GeometricSolutions 0.6 a
# solution carries the time series and the vector field alongside `q` and `p`.
function ParametricDataLoader(ensemble_solution::EnsembleSolution{T,
        T1,
        Vector{ST}}) where {T,
        T1,
        TuT,
    ...
    data = (q = zeros(T, sys_dim, input_time_steps, n_params),
        p = zeros(T, sys_dim, input_time_steps, n_params))
...
function optimize_for_one_epoch!(opt::Optimizer,
    ...
    for batch_indices in batches
        ...
        _input_nt_output_nt_parameter_indices = convert_input_and_batch_indices_to_array(
            dl, batch, batch_indices)
...
function convert_input_and_batch_indices_to_array(dl::ParametricDataLoader{T, BT},
        batch::Batch,
    ...
    parameter_indices(dl, batch_indices)
end
...
function (batch::Batch)(dl::ParametricDataLoader)
    batch_over_two_axes(
        ...
        dl.n_params, number_of_batches(dl, batch))
end

function (o::Optimizer)(nn::NeuralNetwork{<:GeneralizedHamiltonianArchitecture},
        dl::ParametricDataLoader, batch::Batch{:FeedForward},
        n_epochs::Integer = 1, loss::NetworkLoss = ParametricLoss(); kwargs...)
