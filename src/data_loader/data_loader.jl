...
function DataLoader(data::AbstractArray{T, 3}, target::AbstractVector{T1}, batch_size=32, patch_length=7) where {T, T1} 
    @info "You provided a tensor and a vector as input. This will be treated as a classification problem. Tensor axes: (i) & (ii) image axes and (iii) batch dimesnion."
    ...
    DataLoader{T, typeof(data), typeof(target), typeof(batch_output)}(data_preprocessed, batch_input, target, batch_output, patch_length^2, number_of_patches, batch_size, 1, length(target), number_of_patches)
end
