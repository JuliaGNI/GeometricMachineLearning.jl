...
"""
struct ParametricResNetLayer{M, N, F1 <: Activation, PT, ReturnParameters} <:
...
function ParametricResNetLayer(dim::Integer, width::Integer, activation = identity;
        parameters::OptionalParameters = NullParameters(), return_parameters::Bool)
    flat_parameters, layout = _flatten_system_parameters(parameters)
    _activation = Activation(activation)
    ParametricResNetLayer{dim, dim, typeof(_activation), typeof(layout), return_parameters}(
        width, _activation, length(flat_parameters), layout)
end

function initialparameters(
        rng::Random.AbstractRNG, init_weight::AbstractNeuralNetworks.Initializer,
        l::ParametricResNetLayer{M, M}, backend::KernelAbstractions.Backend,
...
function parameterlength(l::ParametricResNetLayer{M, M}) where {M}
    (l.width + l.parameter_length) * (M + 1) + M * (l.width + 1)
end

function (d::ParametricResNetLayer{M, M, F, PT, false})(
        x::AbstractVecOrMat, problem_params::OptionalParameters,
        ps::NamedTuple) where {M, F, PT}
    input = concatenate_array_with_parameters(x, problem_params)
    x + d.activation.(ps.downscale_weight *
                  d.activation.(ps.upscale_weight * input .+ ps.upscale_bias) .+ ps.bias)
end

function (d::ParametricResNetLayer{M, M, F, PT, true})(
        x::AbstractVecOrMat, problem_params::OptionalParameters,
        ps::NamedTuple) where {M, F, PT}
    input = concatenate_array_with_parameters(x, problem_params)
    (
        x + d.activation.(ps.downscale_weight *
                      d.activation.(ps.upscale_weight * input .+ ps.upscale_bias) .+
                      ps.bias),
        problem_params)
end

function (d::ParametricResNetLayer)(input::Tuple, ps::NamedTuple)
    length(input) == 2 ? d(input..., ps) :
    error("The tuple must contain the input array/nt as well as the system parameters.")
end

function (d::ParametricResNetLayer{M, M, F, PT, false})(
        z::QPT, problem_params::OptionalParameters, ps::NamedTuple) where {M, F, PT}
    @assert iseven(M)
