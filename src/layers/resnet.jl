...
function initialparameters(backend::KernelAbstractions.Backend, T::Type, ::ResNetLayer{M, M, use_bias}; rng::Random.AbstractRNG=Random.default_rng(), init_weight = GlorotUniform(), init_bias = ZeroInitializer()) where {M, use_bias}
    if use_bias
