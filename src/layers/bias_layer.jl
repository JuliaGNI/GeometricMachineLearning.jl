...
function initialparameters(rng::AbstractRNG, init_bias::AbstractNeuralNetworks.Initializer,
        ::BiasLayer{M, M}, backend::Backend, ::Type{T}) where {M, T}
...
function (::BiasLayer{M, M})(z::NamedTuple{(:q, :p), Tuple{AT, AT}},
        ps::NamedTuple) where {
        M,
        AT <: AbstractVector
}
...
function (::BiasLayer{M, M})(z::NamedTuple{(:q, :p), Tuple{BT, BT}},
        ps::NamedTuple) where {
        M,
