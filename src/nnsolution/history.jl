...
struct SingleHistory{TP <: TrainingParameters, TD, TL}
    ...
    function SingleHistory(parameters, datashape, size_data, loss)
        new{typeof(parameters), typeof(datashape), typeof(loss)}(parameters, datashape, size_data, loss)
    end
end
...
function Base.iterate(history::History, state = 1)
    state > size(history) ? nothing : (history[state], state+1)
end
