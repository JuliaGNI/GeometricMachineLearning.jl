...
struct NeuralNetSolution{
    TNN <: NeuralNetwork, TP <: AbstractProblem, timestep <: Union{Nothing, Real}, TL}
    nn::TNN
    problem::TP
    timestep::timestep
    loss::TL
    ...
    function NeuralNetSolution(nn::NeuralNetwork, sh::SingleHistory, loss,
            problem::AbstractProblem = UnknownProblem,
            timestep::Real = isUnknown(problem) ? nothing : timestep(problem))
        new{typeof(nn), typeof(problem), typeof(timestep), typeof(loss)}(
            nn, problem, timestep, loss, History(sh))
    end

    function NeuralNetSolution(
            nn::NeuralNetwork, h::History, loss, problem::AbstractProblem = UnknownProblem,
            timestep::Real = isUnknown(problem) ? nothing : timestep(problem))
        new{typeof(nn), typeof(problem), typeof(timestep), typeof(loss)}(
            nn, problem, timestep, loss, h)
    end
end
...
@inline problem(nns::NeuralNetSolution{
    T, <:EnsembleProblem} where {T}) = GeometricEquations.problem(nns.problem, 1)
...
function set_sizemax_history(nns::NeuralNetSolution, sizemax::Int)
    _set_sizemax_history(nns.history, sizemax)
end
