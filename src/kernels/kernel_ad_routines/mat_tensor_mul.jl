...
function ChainRulesCore.rrule(::typeof(mat_tensor_mul), B::AbstractMatrix{T}, A::AbstractArray{
        T, 3}) where {T}
...
@kernel function lower_da_kernel!(dA::AT, S::AbstractVector{T}, dC::AT) where {
        T, AT <: AbstractArray{T, 3}}
    l, m, h = @index(Global, NTuple)

    temp = zero(T)
    for i in (l + 1):size(dA, 1)
...
@kernel function lower_ds_kernel!(dS::AbstractMatrix{T}, A::AT, dC::AT) where {
        T, AT <: AbstractArray{T, 3}}
    ...
    for j in axes(A, 2)
        for i in 2:n
            i_sum = (i - 2) * (i - 1) ÷ 2
            temp = (i_sum < l < (i_sum + i)) ? temp + A[l - i_sum, j, h] * dC[i, j, h] :
                   temp
        end
    end
...
function ChainRulesCore.rrule(::typeof(lo_mat_mul), S::AbstractVector{T},
        A::AbstractArray{T, 3}, n::Int) where {T}
    C = lo_mat_mul(S, A, n)
...
@kernel function upper_da_kernel!(dA::AT, S::AbstractVector{T}, dC::AT) where {
        T, AT <: AbstractArray{T, 3}}
    l, m, h = @index(Global, NTuple)
...
@kernel function upper_ds_kernel!(dS::AbstractMatrix{T}, A::AT, dC::AT) where {
        T, AT <: AbstractArray{T, 3}}
    l, h = @index(Global, NTuple)
    n = size(A, 1)
...
function ChainRulesCore.rrule(::typeof(up_mat_mul), S::AbstractVector{T},
        A::AbstractArray{T, 3}, n::Int) where {T}
    C = up_mat_mul(S, A, n)
...
function ChainRulesCore.rrule(::typeof(skew_mat_mul), S::AbstractVector{T},
        A::AbstractArray{T, 3}, n::Int) where {T}
    ...
    function skew_mat_mul_pullback(dC)
        ...
        return f̄, reshape(sum(dS_lower - dS_upper, dims = 2), length(S)),
        dA_lower - dA_upper, NoTangent()
    end

    return C, skew_mat_mul_pullback
end

function ChainRulesCore.rrule(::typeof(mat_tensor_mul), B::SkewSymMatrix{T}, A::AbstractArray{
        T, 3}) where {T}
    @assert size(A, 1) == B.n
    C = mat_tensor_mul(B, A)
...
@kernel function symmetric_da_kernel!(dA::AT, S::AbstractVector{T}, dC::AT) where {
        T, AT <: AbstractArray{T, 3}}
    l, m, h = @index(Global, NTuple)
    temp = zero(T)
    for i in l:size(dA, 1)
        i_sum = (i - 1) * i ÷ 2
...
@kernel function symmetric_ds_kernel!(dS::AbstractMatrix{T}, A::AT, dC::AT) where {
        T, AT <: AbstractArray{T, 3}}
    l, h = @index(Global, NTuple)
    temp = zero(T)
    for i in axes(dC, 1)
...
function ChainRulesCore.rrule(::typeof(symmetric_mat_mul), S::AbstractVector{T},
        A::AbstractArray{T, 3}, n::Int) where {T}
...
function ChainRulesCore.rrule(::typeof(mat_tensor_mul), B::SymmetricMatrix{T}, A::AbstractArray{
        T, 3}) where {T}
    @assert size(A, 1) == B.n
    C = mat_tensor_mul(B, A)
