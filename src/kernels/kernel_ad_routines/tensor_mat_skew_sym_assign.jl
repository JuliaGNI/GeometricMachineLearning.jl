@kernel function dz_kernel!(dZ::AT, Z::AT, A::AbstractMatrix{T}, dB::AT, sys_dim::Int,
...
@kernel function da_kernel!(dA::AT, Z::AT, ::BT, dB::AT, ::Int,
        seq_length::Int) where {T, AT <: AbstractArray{T, 3}, BT <: AbstractMatrix{T}}
    m, n, h = @index(Global, NTuple)

    temp = zero(T)
    for j in 1:seq_length
        for i in (j + 1):seq_length
...
function ChainRulesCore.rrule(::typeof(tensor_mat_skew_sym_assign),
        Z::AbstractArray{T, 3}, A::AbstractArray{T, 2}) where {T}
