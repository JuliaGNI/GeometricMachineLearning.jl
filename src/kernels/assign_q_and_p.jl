...
@kernel function assign_first_half!(q::AbstractArray{T, 3}, x::AbstractArray{
        T, 3}) where {T}
    i, j, k = @index(Global, NTuple)
    q[i, j, k] = x[i, j, k]
end

@kernel function assign_second_half!(
        p::AbstractArray{T, 3}, x::AbstractArray{
            T, 3}, N::Integer) where {T}
    i, j, k = @index(Global, NTuple)
    p[i, j, k] = x[i + N, j, k]
end
