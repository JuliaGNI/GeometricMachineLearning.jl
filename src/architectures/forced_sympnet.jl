...
struct ForcedSympNet{FT, AT} <: NeuralNetworkIntegrator
    ...
    function ForcedSympNet(dim::Integer;
        ...
        new{forcing_type, typeof(activation)}(
            dim, upscaling_dimension, n_layers, n_forcing_layers, activation, init_upper)
    end
    ...
    function ForcedSympNet(dl::DataLoader;
            ...
            init_upper = g_init_upper_default,
            forcing_type::Symbol = :P)
        new{forcing_type, typeof(activation)}(dl.input_dim, upscaling_dimension, n_layers,
            n_forcing_layers, activation, init_upper)
    end
end
...
function Chain(arch::ForcedSympNet{FT}) where {FT}
    ...
    for i in 1:arch.n_layers
        layers = if is_upper_criterion(i)
            (layers..., GradientLayerQ(arch.dim, arch.upscaling_dimension, arch.act))
        else
            (layers...,
                ForcingLayer(arch.dim, arch.upscaling_dimension, arch.n_forcing_layers,
                    arch.act; return_parameters = false, type = FT),
