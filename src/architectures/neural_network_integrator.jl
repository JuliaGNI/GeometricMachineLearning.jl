...
function Base.iterate(nn::NeuralNetwork{<:NeuralNetworkIntegrator}, ics::AT;
...
function Base.iterate(nn::NeuralNetwork{<:NeuralNetworkIntegrator},
    ...
    # Array to store the predictions
    valuation = (q = KernelAbstractions.allocate(backend, T, n_dim2, n_points),
        p = KernelAbstractions.allocate(backend, T, n_dim2, n_points))
