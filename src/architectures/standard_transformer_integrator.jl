...
function StandardTransformerIntegrator(sys_dim::Int; transformer_dim::Int = sys_dim,
    ...
    StandardTransformerIntegrator(
        ...
        resnet_activation, attention_activation, add_connection)
end

function Chain(arch::StandardTransformerIntegrator)
    layers = arch.sys_dim == arch.transformer_dim ? () :
             (Dense(arch.sys_dim, arch.transformer_dim, arch.upsacling_activation),)
    for _ in 1:arch.L
        layers = (layers...,
            MultiHeadAttention(
                arch.transformer_dim, arch.n_heads; add_connection = arch.add_connection,
                activation = arch.attention_activation))
        layers = (layers...,
            Chain(ResNet(arch.transformer_dim, arch.n_blocks, arch.resnet_activation)).layers...)
    end
    layers = arch.sys_dim == arch.transformer_dim ? layers :
             (layers..., Dense(arch.transformer_dim, arch.sys_dim, identity))
