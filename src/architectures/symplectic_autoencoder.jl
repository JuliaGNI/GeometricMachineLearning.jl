...
function SymplecticAutoencoder(full_dim::Integer, reduced_dim::Integer; n_encoder_layers::Integer = 4,
    ...
    if encoder_init_q && decoder_init_q
        SymplecticAutoencoder{:EncoderInitQ, :DecoderInitQ, typeof(activation)}(full_dim, reduced_dim, n_encoder_layers, n_encoder_blocks, n_decoder_layers, n_decoder_blocks, n_decoder_output_layers, sympnet_upscale, activation)
    elseif encoder_init_q && !decoder_init_q
        SymplecticAutoencoder{:EncoderInitQ, :DecoderInitP, typeof(activation)}(full_dim, reduced_dim, n_encoder_layers, n_encoder_blocks, n_decoder_layers, n_decoder_blocks, n_decoder_output_layers, sympnet_upscale, activation)
    elseif !encoder_init_q && decoder_init_q
        SymplecticAutoencoder{:EncoderInitP, :DecoderInitQ, typeof(activation)}(full_dim, reduced_dim, n_encoder_layers, n_encoder_blocks, n_decoder_layers, n_decoder_blocks, n_decoder_output_layers, sympnet_upscale, activation)
    elseif !encoder_init_q && !decoder_init_q
        SymplecticAutoencoder{:EncoderInitP, :DecoderInitP, typeof(activation)}(full_dim, reduced_dim, n_encoder_layers, n_encoder_blocks, n_decoder_layers, n_decoder_blocks, n_decoder_output_layers, sympnet_upscale, activation)
    end
end
...
function _append_decoder_output_layers(arch::SymplecticAutoencoder, layers::Tuple, _determine_layer_type)
    for layer_index in 1:arch.n_decoder_output_layers
        layers = _determine_layer_type(layer_index) ?
            (layers..., GradientLayerQ(arch.full_dim, arch.sympnet_upscale * arch.full_dim, arch.activation)) :
            (layers..., GradientLayerP(arch.full_dim, arch.sympnet_upscale * arch.full_dim, arch.activation))
    end
    layers
end
...
function decoder(nn::NeuralNetwork{<:SymplecticAutoencoder})
    arch = NonLinearSymplecticDecoder(nn.architecture.full_dim, nn.architecture.reduced_dim, nn.architecture.n_decoder_layers, nn.architecture.n_decoder_blocks, nn.architecture.n_decoder_output_layers, nn.architecture.sympnet_upscale, nn.architecture.activation)
