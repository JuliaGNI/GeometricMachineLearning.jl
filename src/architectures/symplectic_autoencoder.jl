...
- `n_decoder_output_layers::Integer = 0`: The number of additional nonlinear SympNet gradient layers appended after the final (linear) PSD layer of the decoder. These layers operate at `full_dim` and provide extra expressivity to the decoder. When set to `0` (the default) the decoder ends with the standard linear PSD layer.
...
function SymplecticAutoencoder(
        full_dim::Integer, reduced_dim::Integer; n_encoder_layers::Integer = 4,
        n_encoder_blocks::Integer = 2,
        n_decoder_layers::Integer = 1,
        n_decoder_blocks::Integer = 3,
        n_decoder_output_layers::Integer = 0,
    ...
    if encoder_init_q && decoder_init_q
        SymplecticAutoencoder{:EncoderInitQ, :DecoderInitQ, typeof(activation)}(
            full_dim, reduced_dim, n_encoder_layers, n_encoder_blocks, n_decoder_layers,
            n_decoder_blocks, n_decoder_output_layers, sympnet_upscale, activation)
    elseif encoder_init_q && !decoder_init_q
        SymplecticAutoencoder{:EncoderInitQ, :DecoderInitP, typeof(activation)}(
            full_dim, reduced_dim, n_encoder_layers, n_encoder_blocks, n_decoder_layers,
            n_decoder_blocks, n_decoder_output_layers, sympnet_upscale, activation)
    elseif !encoder_init_q && decoder_init_q
        SymplecticAutoencoder{:EncoderInitP, :DecoderInitQ, typeof(activation)}(
            full_dim, reduced_dim, n_encoder_layers, n_encoder_blocks, n_decoder_layers,
            n_decoder_blocks, n_decoder_output_layers, sympnet_upscale, activation)
    elseif !encoder_init_q && !decoder_init_q
        SymplecticAutoencoder{:EncoderInitP, :DecoderInitP, typeof(activation)}(
            full_dim, reduced_dim, n_encoder_layers, n_encoder_blocks, n_decoder_layers,
            n_decoder_blocks, n_decoder_output_layers, sympnet_upscale, activation)
    end
end
...
function encoder_or_decoder_layers_from_iteration(
        arch::SymplecticAutoencoder, encoder_iterations::AbstractVector{<:Integer},
        n_encoder_layers::Integer, _determine_layer_type)
    encoder_layers = ()
    ...
    for (i, it) in zip(axes(encoder_iterations_reduced, 1), encoder_iterations_reduced)
        for layer_index in 1:n_encoder_layers
            encoder_layers = _determine_layer_type(layer_index) ?
                             (encoder_layers...,
                GradientLayerQ(it, arch.sympnet_upscale * it, arch.activation)) :
                             (encoder_layers...,
                GradientLayerP(it, arch.sympnet_upscale * it, arch.activation))
        end
...
function encoder_layers_from_iteration(arch::SymplecticAutoencoder{:EncoderInitQ},
        encoder_iterations::AbstractVector{<:Integer})
    encoder_or_decoder_layers_from_iteration(arch, encoder_iterations, arch.n_encoder_layers, isodd)
end

function encoder_layers_from_iteration(arch::SymplecticAutoencoder{:EncoderInitP},
        encoder_iterations::AbstractVector{<:Integer})
    encoder_or_decoder_layers_from_iteration(arch, encoder_iterations, arch.n_encoder_layers, iseven)
end

function _append_decoder_output_layers(arch::SymplecticAutoencoder, layers::Tuple, _determine_layer_type)
    for layer_index in 1:arch.n_decoder_output_layers
        layers = _determine_layer_type(layer_index) ?
                 (layers...,
            GradientLayerQ(arch.full_dim, arch.sympnet_upscale * arch.full_dim, arch.activation)) :
                 (layers...,
            GradientLayerP(arch.full_dim, arch.sympnet_upscale * arch.full_dim, arch.activation))
    end
    layers
end

function decoder_layers_from_iteration(arch::SymplecticAutoencoder{<:Any, :DecoderInitQ},
        decoder_iterations::AbstractVector{<:Integer})
    layers = encoder_or_decoder_layers_from_iteration(arch, decoder_iterations, arch.n_decoder_layers, isodd)
    _append_decoder_output_layers(arch, layers, isodd)
end

function decoder_layers_from_iteration(arch::SymplecticAutoencoder{<:Any, :DecoderInitP},
        decoder_iterations::AbstractVector{<:Integer})
    layers = encoder_or_decoder_layers_from_iteration(arch, decoder_iterations, arch.n_decoder_layers, iseven)
    _append_decoder_output_layers(arch, layers, iseven)
end

function encoder(nn::NeuralNetwork{<:SymplecticAutoencoder})
    arch = NonLinearSymplecticEncoder(
        nn.architecture.full_dim, nn.architecture.reduced_dim,
        nn.architecture.n_encoder_layers, nn.architecture.n_encoder_blocks,
        nn.architecture.sympnet_upscale, nn.architecture.activation)
...
function decoder(nn::NeuralNetwork{<:SymplecticAutoencoder})
    arch = NonLinearSymplecticDecoder(
        nn.architecture.full_dim, nn.architecture.reduced_dim,
        nn.architecture.n_decoder_layers, nn.architecture.n_decoder_blocks,
        nn.architecture.n_decoder_output_layers,
        nn.architecture.sympnet_upscale, nn.architecture.activation)
