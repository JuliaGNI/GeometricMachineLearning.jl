...
struct ClassificationTransformer{AT} <: Architecture 
    ...
    function ClassificationTransformer(dl::DataLoader{T, BT, CT}; n_heads::Int=7, L::Int=16, activation::AT=softmax, Stiefel::Bool=true, add_connection::Bool=true) where {T, T1, BT<:AbstractArray{T, 3}, CT<:AbstractArray{T1, 3}, AT}
        new{AT}(dl.input_dim, dl.output_dim, n_heads, L, activation, Stiefel, add_connection)
    end
end

function Chain(arch::ClassificationTransformer)
    Chain(
        Transformer(arch.transformer_dim, arch.n_heads, arch.L, Stiefel=arch.Stiefel, add_connection=arch.add_connection).layers...,
