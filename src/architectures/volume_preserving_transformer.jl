...
"""
function VolumePreservingTransformer(sys_dim::Int, transformer_dim::Int = sys_dim, upscaling_dimension::Int = 2 * transformer_dim, L::Int = 2, activation = tanh, init_upper::Bool=true)
    VolumePreservingTransformer{typeof(tanh), init_upper}(sys_dim, transformer_dim, upscaling_dimension, L, activation)
end

function Chain(arch::VolumePreservingTransformer{AT, true}) where {AT}
    layers = (PSDLayer(arch.sys_dim, arch.transformer_dim), )
    for _ in 1:arch.L 
        layers = (layers..., VolumePreservingAttention(arch.transformer_dim))
